...
const ContactPageTemplate = ({ contacts }) => {
  ...
  useEffect(() => {
    if (!prevBackground) {
      document.querySelector('#container.background').style.backgroundImage = `url(${background})`
    }
        ...
        <div className="wrapper">
          ...
          <ul id="contact">
            { contact.map((entry, index) => {
              return (
                <li key={ index }><a href={ entry.url }>{ entry.name }</a></li>
              )
            })}
          </ul>
        ...
        <div className="wrapper">
          ...
          <ul id="community">
            { community.map((entry, index) => {
              return (
                <li key={ index }><a href={ entry.url } target="_blank" rel="noreferrer">{ entry.name }</a></li>
              )
            })}
          </ul>
...
const ContactPage = ({ data }) => {
  ...
  const contacts = markdownRemark && markdownRemark.frontmatter && markdownRemark.frontmatter.contacts

  return (
    <Layout className="contact background has-columns" style={ { minHeight: "894px" } }>
      <Helmet>
        <title>Contact and Community :: PPF House</title>
        <meta name="description" content="PPF House: Contacts and community links" />
      </Helmet>
      <ContactPageTemplate
        contacts={ contacts }
      />
